...
@section('title',' | ' . __('site.users_earn') .' - ' . __('site.offerwalls'))
...
<div class="row mb-2">
    <div class="card">
        ...
        <div class="card-body p-1">
            <form action="{{ route('admin.userpoints.offer') }}" method="get">
                <div class="row m-2 col-md-12">
                    ...
                    <div class="form-group col-md-6 p-0 m-0 px-2">
                        <label class="py-1 m-0" for="offerwall_id">@lang('site.offerwall')    @lang('site.name')</label>
                        <select class="form-control custom-select select2bs4" id="offerwall_id" name="offerwall_id">
                            <option value="" selected >@lang('site.choose_offerwall')</option>
                            @foreach ($offerwalls as $offerwall)
                            <option value="{{$offerwall->id}}" {{ request()->offerwall_id== $offerwall->id ? 'selected' : '' }}>
                                {{$offerwall->name}}</option>
                            @endforeach
                        </select>
                    </div>
...
<div class="row">
    <div class="card card-light ">
        <div class="card-header  bg-white">
            ...
            <h3 class="card-title">
                @lang('site.users_earn')   @lang('site.offerwalls')
            </h3>
            <h3>
                {{$userpoints->count()}}
            </h3>
                ...
                <table class="table table-bordered table-hover">
                   ...
                   {{-- <span style="display: none;">{{$total=0}}</span>--}}
                        ...
                        <tr style="">
                            ...
                            <td>
                            <a href="{{$offerwall->site_url}}" target="_blank">{{$userpoint->offerwall->name}}</a>
                            </td>
