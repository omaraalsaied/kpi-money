...
@section('title',' | ' . __('site.edit') .' - ' . __('site.adsperm'))
@section('content')
<div class="row mb-2">
    <div class="card">
        <div class="card-header  bg-white">
            <h3 class="card-title">
               @lang('site.edit')    @lang('site.adperm')
            </h3>
        </div>
        <div class="card-body p-1">
            <form action="{{ route('admin.adsperms.update',$adsperm->id) }}" method="post">
                @csrf
                @method('put')
                <div class="row m-2 col-md-12">
                    <div class="form-group col-md-6 p-0 m-0 px-2">
                        ...
                        <select class="form-control  @error('user_id') is-invalid @enderror  custom-select select2bs4" id="user_id" name="user_id">
                            ...
                            <option value="{{$user->id}}" {{  $adsperm->user_id== $user->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group col-md-6 p-0 m-0 px-2">
                        ...
                        <select class="form-control  @error('point_id') is-invalid @enderror  custom-select select2bs4" id="point_id" name="point_id">
                            ...
                            <option value="{{$point->id}}" {{  $adsperm->point_id== $point->id ? 'selected' : '' }}>
                                {{$point->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6 p-0 m-0 px-2">
                        ...
                        <input type="number" name="times" class="form-control @error('times') is-invalid @enderror" placeholder="@lang('site.count')"
                            id="times" value="{{ $adsperm->times }}"min="0" step="1" value="0">
